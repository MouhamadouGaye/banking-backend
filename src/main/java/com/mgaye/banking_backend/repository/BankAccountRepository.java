...
public interface BankAccountRepository extends JpaRepository<BankAccount, UUID> {
        ...
        // @EntityGraph(attributePaths = { "user" })
        // Optional<BankAccount> findByIdWithUser(UUID accountId); // Changed to UUID

        // Replace findByIdWithUser with this:
        @EntityGraph(attributePaths = { "user" })
        @Query("SELECT a FROM BankAccount a WHERE a.id = :accountId")
        Optional<BankAccount> findByIdWithUser(@Param("accountId") UUID accountId);

        boolean existsByIdAndUser(UUID accountId, User user); // Changed to UUID
        ...
        // Specialized queries - changed userId to UUID
        @Query("SELECT a FROM BankAccount a WHERE a.user.id = :userId AND a.status = 'ACTIVE'")
